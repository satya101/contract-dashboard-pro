...
export function normalizeSummary(raw) {
  ...
  // Precompute lot/plan join (uses || internally but not mixed with ?? at top level)
  const lotPlanJoined = (
    [get(s, ["title.lot"]), get(s, ["title.plan"])]
      .filter(Boolean)
      .join(" / ")
  ) || undefined;
  ...
  const disclosures = {
    ...
    planning_zoning:
      sourceDisc.planning_zoning ??
      sourceDisc["planning/zoning"] ??
      sourceDisc.planning ??
      sourceDisc.zoning,
    rates: sourceDisc.rates ?? sourceDisc.council_rates,
    insurance: sourceDisc.insurance ?? sourceDisc.building_insurance,
    outgoings:
      sourceDisc.outgoings ??
      sourceDisc.owners_corp_fees ??
      sourceDisc["owners_corporation"],
    notices:
      sourceDisc.notices ??
      sourceDisc.statements_notices ??
      sourceDisc["notices_or_orders"],
  };
